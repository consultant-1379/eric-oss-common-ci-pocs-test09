...
ARG USER_NAME="eric-oss-common-ci-pocs-test09"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-common-ci-pocs-test09-app.jar
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    ...
    -jar eric-oss-common-ci-pocs-test09-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-common-ci-pocs-test09-jsb \
